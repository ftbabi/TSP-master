...
class Ant(object):
    ...
    def __choice_next_city(self):
        ...
        for i in self.available_cnc:
            if not(self.current_city in self.__cnc_first and i in self.__cnc_first) and not (self.current_city in self.__cnc_second and i in self.__cnc_second):
                try:
    ...
    def search_path(self):
        ...
        while len(self.available_cnc) != 0 and len(self.path) <= 16:
            # 移动到下一个城市
        ...
        self.total_distance /= float(self.clock)
...
class TSP(object):
    ...
    # 初始化
    def new(self, cnc1=[0,1,2,3], evt=None):
    ...
    def search_path(self, evt=None):
        ...
        while self.__running:
            ...
            print u"迭代次数：", self.iter, u"最佳路径总距离：", float(self.best_ant.total_distance), "路径顺序： ", self.best_ant.path, "原始分数：", self.best_ant.total_distance*self.best_ant.clock, "总共时间：", self.best_ant.clock
            # 连线
