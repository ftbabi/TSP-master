...
#             if not(self.current_city in self.__cnc_first and i in self.__cnc_first) and not (self.current_city in self.__cnc_second and i in self.__cnc_second) and self.open_table_city[i]:
#                 try:
...
#                 if j % 2 == 0:
#                     distance_graph[i][j] += time_process[0]
#                 else:
#                     distance_graph[i][j] += time_process[1]
#
#         # distance_graph_bak = copy(distance_graph)
#
#     # 按键响应程序
...
#             print u"迭代次数：", self.iter, u"最佳路径总距离：", int(self.best_ant.total_distance), "路径顺序： ", self.best_ant.path
#             # 连线
